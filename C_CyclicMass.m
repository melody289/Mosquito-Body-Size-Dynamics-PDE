...
%Adg = zeros(astepsA, msteps, (tmax + 1),9);
%Juvg = zeros(astepsJ, msteps, (tmax + 1),9);
%Anewg = zeros((tmax + 1),msteps,9);
...
J1 = Jiv;
 ...
 gj = @(m, tt) g1.*m.^(2/3).*0.5.*(cos(2*pi.*tt./omg(kkj)) +1); 
...
J1(1,1) = J1(1,1) + born;
   ...
   save(sprintf('C1par_cylicmass_%d.mat', omg(kkj)), 'Ad')    
        toc
end
