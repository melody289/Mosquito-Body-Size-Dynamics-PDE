...
mass = m0:dm:(maxm);
%agem2 = find(mass> ((agem-1)*da*gj(m0)+m0-gj(m0)/10) , 1);
...
gj = @(mm, tt) g1.*mm.^(2/3); 
rt = @(tt,mm) 0.25.*(maxm-mm).*(1- 0.5.*(cos(2*pi.*tt./omg(kkj)) +1)).^5./(maxm.*((1- 0.5.*(cos(2*pi.*tt./omg(kkj)) +1)).^5 + 0.75^5)) + 0.02;
...
%Jnewd(round(i/dt)) = born;
    ...
    if(mod(round((i + 0.001),2),tm1) ==0)
        ...
        save('C1par_cyclicDeath.mat',  'Adg')
        toc
end
